...
parser.add_argument(
    "--batch_size", type=int, default=1, help="Batch size for mini-batch training."
)
...
_, unlabel_xs, key_list,_ = get_dataset_dataloader(
    args.dataset_name, batch_size=args.batch_size, num_workers=1
)
...
for j, key in enumerate(key_list):
    new_key = ind2line[key]
    line2likeness[new_key] = [unlab[j]]
    new_key_list.append(new_key)
new_key_list.sort(key=lambda x: line2likeness[x][0], reverse=True)


test_likeness_list = []
with open(f"results/{args.output_name}", "w") as f:
    for line in new_key_list:
        likeness = line2likeness[line][0]  # 直接获取单一值
        f.write(f"{line} {likeness}\n")
        test_likeness_list.append(likeness)
        # print(likeness)
