...
if __name__ == "__main__":
    ...
    # filenames = [f"train_clustering_{i}.txt" for i in range(5)]
    # filenames += ["test_clustering.txt"]
    filenames = ['data/test_one.txt']
    for i in range(len(filenames)):
        print(filenames[i],'filenames[i]')
        make_npz_file(filenames[i], f'{filenames[i].split(".")[0]}.npz')
