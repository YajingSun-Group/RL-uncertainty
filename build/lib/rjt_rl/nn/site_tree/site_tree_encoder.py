...
class SiteTreeEncoder(TreeEncoder):
    ...
    def forward(
        ...
        # print(len(root_batch),'root_batch')
        ...
        for t in range(max_depth):
            ...
            cur_h_nei = []
            cur_site = []
            # print(len(prop_list),'prop_list',prop_list[0])
            for node_x, node_y in prop_list:
                x, y = node_x.safe_idx, node_y.safe_idx
                cur_x.append(node_x.safe_wid) #vocab id
                # print(node_x.safe_wid,'node_x.safe_wid')
                ...
                for node_z in node_x.neighbors:
                    ...
                    # print(self.embed_site.encode_site_node(node_z, node_x),'site')

                h_nei = pad_nodes(h_nei, padding, MAX_NB)
                cur_h_nei.extend(h_nei)

                site_t = pad_nodes(site, self.embed_site.padding, MAX_NB)
                # print(np.shape(site_t),'site_t',site_t)
                cur_site.append(site_t)
            # if 2 in cur_x:
            #     print("1 exists in the tensor.")
            cur_x_t = index_tensor(cur_x, device)
            # print(np.shape(cur_x_t),'cur_x_t')
            cur_x_te = self.embedding(cur_x_t)
            # print(np.shape(cur_x_te),'cur_x_te')
            cur_h_nei_t = torch.cat(cur_h_nei, dim=0).view(-1, MAX_NB, self.hidden_size)

            cur_site_t = index_tensor(cur_site, device)
            # print(np.shape(cur_site_t),'cur_site_t',cur_site_t[0])
            cur_site_te = self.embed_site(cur_site_t)
            # print(np.shape(cur_site_te),'cur_site_te',cur_site_te[0])#,np.sum(cur_site_te.detach().numpy()))
            # if torch.any(cur_site_te == -1):
            #     print("1 exists in the tensor.")
