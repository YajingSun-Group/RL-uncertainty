...
CONFIG_YAML=$SCRIPT_DIR/config_SA_rdkit.yaml ##
DATASET_DIR=$TOP_DIR/data
MODEL_DIR=$TOP_DIR/GNN_finetune_model
OUTDIR=$TOP_DIR/results/config_SA_rdkit/ ##
...
$PYTHON $TOP_DIR/scripts/rl/train_ppo.py \
    ...
    > $OUTDIR/config_SA_rdkit.log 2>&1 ##

END_TIME=$(date +%s)
ELAPSED_TIME=$((END_TIME - START_TIME))
echo "Script execution time: $ELAPSED_TIME seconds" >> $OUTDIR/config_SA_rdkit.log ##