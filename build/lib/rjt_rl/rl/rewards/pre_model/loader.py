...
def mol_to_graph_data_obj_simple_2(mol,sol):
    # atoms
    ...
    sol_atom_features_list = []
    for sol_atom in sol.GetAtoms():
        sol_atom_feature = [allowable_features['possible_atomic_num_list'].index(
            sol_atom.GetAtomicNum())] + [allowable_features[
            'possible_chirality_list'].index(sol_atom.GetChiralTag())]
        sol_atom_features_list.append(sol_atom_feature)
    sol_x = torch.tensor(np.array(sol_atom_features_list), dtype=torch.long)
    ...
    if len(sol.GetBonds()) > 0: # mol has bonds
        sol_edges_list = []
        sol_edge_features_list = []
        for sol_bond in sol.GetBonds():
            sol_i = sol_bond.GetBeginAtomIdx()
            sol_j = sol_bond.GetEndAtomIdx()
            sol_edge_feature = [allowable_features['possible_bonds'].index(
                sol_bond.GetBondType())] + [allowable_features[
                                            'possible_bond_dirs'].index(
                sol_bond.GetBondDir())]
            sol_edges_list.append((sol_i, sol_j))
            sol_edge_features_list.append(sol_edge_feature)
            sol_edges_list.append((sol_j, sol_i))
            sol_edge_features_list.append(sol_edge_feature)

        # data.edge_index: Graph connectivity in COO format with shape [2, num_edges]
        sol_edge_index = torch.tensor(np.array(sol_edges_list).T, dtype=torch.long)

        # data.edge_attr: Edge feature matrix with shape [num_edges, num_edge_features]
        sol_edge_attr = torch.tensor(np.array(sol_edge_features_list),
                                 dtype=torch.long)
    else:   # mol has no bonds
        sol_edge_index = torch.empty((2, 0), dtype=torch.long)
        sol_edge_attr = torch.empty((0, num_bond_features), dtype=torch.long)

    x_combined = torch.cat([x, sol_x], dim=0)  # 拼接节点特征，假设 x 和 sol_x 的维度匹配
    edge_index_combined = torch.cat([edge_index, sol_edge_index + x.size(0)], dim=1)  # 通过增加偏移量拼接
    edge_attr_combined = torch.cat([edge_attr, sol_edge_attr], dim=0)
...
class MoleculeDataset(InMemoryDataset):
    def __init__(self,
        ...
        """
        self.single_smiles = single_smiles
        # self.dataset = dataset
        # self.root = root

        # super(MoleculeDataset, self).__init__(root, transform, pre_transform,pre_filter)
        super(MoleculeDataset, self).__init__(transform, pre_transform,pre_filter)
    ...
    def process(self):
        data_smiles_list = []
        data_list = []
        if self.single_smiles is not None:
            rdkit_mol = Chem.MolFromSmiles(self.single_smiles)
            data = mol_to_graph_data_obj_simple(rdkit_mol)
            data_list.append(data)
            data_smiles_list.append(self.single_smiles)
...
def _load_FORMAT_dataset(input_path):
    """
    :param input_path:
...
def _load_sider_dataset(input_path):
    ...
    assert len(smiles_list) == len(rdkit_mol_objs_list)
    assert len(smiles_list) == len(labels)
    return smiles_list, rdkit_mol_objs_list, labels.to_numpy()#labels.value
...
def create_all_datasets():
    #### create dataset
    downstream_dir = ['TADF_lite','FORMAT',
            'bace',
            'bbbp',
            'clintox',
            'esol',
